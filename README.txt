audio_hdmi_hd4000
============
OS X Ivy Bridge/7 Series/Socket 1155 - HD4000/AMD/Nvidia HDMI Audio
With or Without DSDT Edits

Update: v2 - 8/15/2014

HDMI audio for OS X HD4000 systems with either: 1. HDMI audio edited dsdt or 2. the HDMI audio ssdt. Both techniques enable native Ivy Bridge graphics power management.  

Details
[Guide]_HD4000-hdmi_audio_(dsdt_or_ssdt)_v2.pdf (above)
[Guide]_NUC-hdmi_audio_(dsdt)_v2.pdf (above
[Guide]_OS_X_hdmi_audio-dsdt_v2, see https://github.com/toleda/audio_hdmi_guides
[Guide]_OS_X_hdmi_audio-ssdt_v2, see https://github.com/toleda/audio_hdmi_guides

OS X HD4000/AMD/Nvidia HDMI Audio (Audio ID: 3 = patch-A3 = filename-3, etc.)

Problem Reporting
1. Required info:
   1. [Guide]_HD4000-hdmi_audio_(dsdt_or_ssdt)_v2.pdf
2. Post to:
   1. http://www.insanelymac.com/forum/topic/292999-mavericks-applehda-hdmi-audio/
   2. http://www.tonymacx86.com/hdmi-audio/112469-mavericks-hdmi-audio-applehda.html

Credit:
PikeRAlpha https://pikeralpha.wordpress.com/2013/09/19/haswell-hdau-solution/
bcc9 Post #11 - http://www.insanelymac.com/forum/topic/290783-intel-hd-graphics-4600-haswell-working-displayport/?p=1934889

toleda
https://github.com/toleda/audio_hdmi_hd4000
README.txt